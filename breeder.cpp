    ...
    for (int i = 0; i < num_generations; i++)
    {
        ...
        for (int j = 0; j < num_species; j++)
        {
            for (int k = 0; k < num_games; k++)
            {
                ...
                Species *species2 = &species[rand() % num_species];
                Board board(9, 9, 2);
                ...
                while (!board.is_finished())
                {
                    ...
                    Move move = board.get_best_move(6, 2, id, id == my_id ? species1->data.data() : species2->data.data());
                    board.do_move(move);
                }
        ...
        for (int j = 0; j < num_species; j++)
        {
            for (int k = 0; k < num_games; k++)
            {
                Species *species1 = &species[j];
                Species *species2 = &species[rand() % num_species];
                Species *species3 = &species[rand() % num_species];
                Board board(9, 9, 3);
                ...
                {
                    id = board.get_next_id(id);
                    Move move = board.get_best_move(5, 2, id, id == my_id ? species1->data.data() : (id == (my_id + 1) % 3 ? species2->data.data() : species3->data.data()));
                    board.do_move(move);
                }
